...
An app can determine if a network is whitelisted or not by sending a `wallet_switchEthereumChain` request for that network. If error code 4902 is returned, then the network is not supported by default by the client wallet.
