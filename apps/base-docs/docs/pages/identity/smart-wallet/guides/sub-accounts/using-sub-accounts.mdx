...
To send transactions with a Sub Account, we'll use Wagmi's `useSendTransaction` hook. Remember that transactions will be limited by the Spend Limits we configured in the [setup](/identity/smart-wallet/guides/sub-accounts/setup):
...
All transactions will be automatically limited by the Spend Limits configured in your [Wagmi setup](/identity/smart-wallet/guides/sub-accounts/setup).
