  ...
  const renderFAQItem = useCallback(
    (item: FAQItem, i: number) => {
      ...
      return (
        <div
          key={questionId}
          className="overflow-hidden rounded-lg bg-dark-palette-backgroundAlternate"
        >
          <button
            type="button"
            className="hover:bg-gray-900 group flex w-full items-center justify-between p-6 text-left transition-colors"
            onClick={getItemClickHandler(i)}
          >
            <div className="flex items-center gap-3">
              <div
                className={`origin-center transform transition-transform duration-200 ease-in-out ${
                  openIndex === i ? 'rotate-90' : ''
                }`}
              >
                <Icon name="chevronRight" width={16} height={16} color="white" />
              </div>
  ...
  return (
    <div className="mx-auto w-full">
      <div className="flex flex-col gap-8 md:flex-row md:gap-24">
        ...
        <div className="flex flex-col gap-4 md:w-2/3">{FAQ_ITEMS.map(renderFAQItem)}</div>
