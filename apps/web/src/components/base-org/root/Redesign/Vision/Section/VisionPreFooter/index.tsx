...
void main() {
  ...
  if (adjustedUV.x < 0.0 || adjustedUV.x > 1.0 || adjustedUV.y < 0.0 || adjustedUV.y > 1.0) {
    // render transparent for out-of-bounds areas
  ...
  if (u_isHovered < 0.001) {
    ...
    // replace black artifacts with white
  ...
  if (smoothedLum < 0.2) {
    ...
    // Render white background instead of black
  ...
  // Final safety check - replace any black pixels with white
  if (gl_FragColor.r < 0.01 && gl_FragColor.g < 0.01 && gl_FragColor.b < 0.01) {
    gl_FragColor = vec4(1.0, 1.0, 1.0, gl_FragColor.a);
  }
}
...
export function VisionPreFooter() {
  ...
  return (
    <section className="relative grid grid-cols-9 gap-8 lg:h-[833px]">
      <div
        ref={containerRef}
